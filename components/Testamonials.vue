<template>
  <!--SLIDER BUTTONS-->
  <div class="md:flex hidden justify-end homeCenterContainer">
  ...
  <div class="bg-gradient-to-b from-white to-wierdGreen h-auto">
    ...
    <div class="homeCenterContainer mt-5">
      <UCarousel ref="reviewCarouselRef" :items="reviewItems" :ui="{
        item: 'basis-full snap-end',
        container: '!h-full rounded-lg shadow-xl',
        indicators: {
          wrapper: 'bottom-0 mt-4 relative mt-[30px]',
        }
      }" class="w-full mx-auto  md:h-[30em] p-0 overflow-hidden" indicators>

        <template #default="{ item }">
          <!-- Render carousel items here -->
          <UIHomeReviewItem :reviewBody="item.reviewBody" :reviewFooter="item.reviewFooter"
        ...
        <template #indicator="{ onClick, page, active }">
          <UButton class="
            ...
            after:bg-[var(--vc-pgn-background-color)] 
            after:w-[var(--vc-pgn-width)] 
            after:block 
            after:h-[var(--vc-pgn-height)]
            ...
            " @click="onClick(page)" :class="active && 'after:!bg-[var(--vc-pgn-active-color)] after:w-[60px] after:rounded-full'
              " disabled />
        </template>
      </UCarousel>
    </div>
